...
class ThermalCalibrationManager:
    ...
    def set_environment(
        ...
        payload = (
            int(emissivity * 100).to_bytes(2, "little")
            + int(distance * 100).to_bytes(2, "little")
            + int(ambient_temp * 100).to_bytes(2, "little")
            + int(humidity * 100).to_bytes(2, "little")
            + int(reflected_temp * 100).to_bytes(2, "little")
        )
